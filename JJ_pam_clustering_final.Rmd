...
#Question is : How can Johnson and Johnson push the L2 and L3 tickets to L1, so that cost is saved.
...
#---Majority undersampling in performed in levelN feature. L1 and L2 are undersampled so as to increase share of L3.
#---Choose 20% points from L2 and 80% points from L1.
...
#--As is seen from the bar charts the distribution of features is similar (in terms of shape) between undersample and full sample.
```{r}
...
#PAM clustering. Choose either sdata or underDF.
#sdata is the full dataset. Use kmax=10. Neither of these clusters have more than 10% membership of L3.
#underDF is the undersampled dataset. Use kmax=20.
...
#This is because it is hard to analyse more than 20 clusters.
```{r}
...
#Define functions which return percentage share of each level for a feature.
```{r}
...
#For the undersample, cluster # 17,18 and 15 are most important since they have high membership of L2 and L3.
# set icl=17,18,15 to make piecharts to answer : What does the typical member of each cluster look like ?
```{r}
